...
if ((!isNaN(parseFloat(days_year)) && isFinite(days_year)) && (days_year>0)){
    ...
    if (days_year >=31 & days_year<= 364) { console.log ('Меньше года');}
    if (days_year >=7 & days_year<= 30)   { console.log ('Меньше месяца');}
    if (days_year >=1 & days_year <= 6)   { console.log ('Меньше недели');}
}
else {
    alert('Вы ввели некорректные данные!!!');
}
